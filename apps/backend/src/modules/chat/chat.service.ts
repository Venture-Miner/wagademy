...
import { CreateInterviewChat, OpenAIChatModel } from '@wagademy/types';
...
export class ChatService {
  ...
  async createChatCompletion(id: string, userId: string, message: string) {
    ...
    if (chat.jobApplication.userId !== userId)
      throw new UnauthorizedException(
        'You cannot modify this chat, it does not belong to you'
      );
    ...
    const history = JSON.parse(chat.history?.toString() as string);
